...
class FlexibleWM:
  ...
  def __init__(self,spec):
    ...
    self.specF['simtime'] = spec.get('simtime',0.4) # Simulation time in sec
    ...
    self.specF['end_stimulation'] = spec.get('end_stimulation',0.3)
    #self.specF['input_strength'] = spec.get('input_strength',10) # strength of the stimulation
    #Qi: change input strength into a vector so that we can control the responses to different features
    self.specF['input_strength'] = numpy.ones((self.specF['N_sensory'], 1))*10#spec.get('input_strength', 10)  # strength of the stimulation
    # selective_stim = np.random.randint(0, self.specF['N_sensory'], 1)
    # self.specF['input_strength'][selective_stim[0]] = 20
    # print(selective_stim[0])
    # print(self.specF['input_strength'][selective_stim[0]])
    # print(self.specF['input_strength'][int(selective_stim-1)])
    ...
    # Make a bump in the input strengths
    selective_stim = 1#numpy.random.randint(0, self.specF['N_sensory'], 1)[0]
    self.specF['selective_stim'] = selective_stim
    selective_ind = numpy.arange(int(round(selective_stim - self.specF['InputWidthFactor'] * self.specF['InputWidth'])),
                        int(round(selective_stim + self.specF['InputWidthFactor'] * self.specF['InputWidth']) + 1), 1)
    selective_scale = scipy.stats.norm.pdf(selective_ind - selective_stim, 0, self.specF['InputWidth'])
    selective_scale /= float(numpy.amax(selective_scale))
    selective_ind = numpy.remainder(selective_ind - 1, self.specF['N_sensory'])
    self.specF['input_strength'][selective_ind] = numpy.expand_dims(10+100 * selective_scale, -1)
  ...
  def give_input_stimulus(self,current_InputCenter) :
    ...
    inp_vect[inp_ind] = self.specF['input_strength'][int(current_InputCenter)]*inp_scale #Qi: modify here to make the input strength stimulus-specific

    #inp_vect[inp_ind] = self.specF['input_strength']*inp_scale
    #inp_vect[:] = inp_vect[:]-numpy.sum(inp_vect[:])/float(inp_vect[:].shape[0]) #Qi This normalizes the activation
    # if current_InputCenter == self.specF['selective_stim']:
    #   print(inp_vect)
    # print(inp_vect[inp_ind])
    #print(inp_vect.shape)
    # print(inp_vect[inp_ind-1000])
    return inp_vect
  ...
  def run_a_trial(self) : 
    ...
    R_rcn = StateMonitor(RCN_pool, 'rate_rnd', record=True, dt=self.specF['window_save_data'] * second)
    ...
    # Qi: edit to save activity in the random network
    RCN_act_enddelay = numpy.ones((self.specF['Number_of_trials'],self.specF['N_random']))*numpy.nan
    RCN_act_begindelay = numpy.ones((self.specF['Number_of_trials'],self.specF['N_random']))*numpy.nan

    # Qi: save sensory network enddelay
    Sensory_act_enddelay = numpy.ones((self.specF['Number_of_trials'],self.specF['N_sensory_pools']*self.specF['N_sensory']))*numpy.nan
    Sensory_act_begindelay = numpy.ones((self.specF['Number_of_trials'],self.specF['N_sensory_pools']*self.specF['N_sensory']))*numpy.nan
    ...
    for index_simulation in range(self.specF['Number_of_trials']) :
      ...
      #Qi: InputCenter is a 1-d vector where the length is N_sensory_pools. And the number in each element indicates
      #the feature that needs to encoded by this sensory pool. Each input center ranges from 0 to N_sensory-1
      # Qi: modify to make the input deterministic on each trial
      # Qi
      InputCenter = numpy.ones(self.specF['N_sensory_pools'])*(index_simulation%256)*2
      ...
      Matrix_pools_receiving_inputs = Matrix_pools[:load]#Qi: only input to the first sensory network #
      print("Load for this trial is "+str(load))
    
      # We build inp_vect, a matrix which gives the stimulus input to each SN #Qi:this is where the input is presented
      ...
      for index_pool in range(self.specF['N_sensory_pools']) :
        if index_pool in Matrix_pools_receiving_inputs :
          ...
          #print(Recurrent_Pools[self.specF['N_sensory']*index_pool:self.specF['N_sensory']*(index_pool+1)].S_ext)
        else :
          Recurrent_Pools[self.specF['N_sensory']*index_pool:self.specF['N_sensory']*(index_pool+1)].S_ext = inp_baseline[index_pool]
      if self.specF['with_random_network'] :
        RCN_pool.S_ext_rnd = inp_baseline_rnd
      ...
      Sensory_act_enddelay[index_simulation, :] = R_rn_enddelay

      R_rn_begindelay = numpy.transpose(R_rn.rate_rec[:, 1].copy())  # beware numpy.transpose is reference
      Sensory_act_begindelay[index_simulation, :] = R_rn_begindelay

      # Also get the results of the random network
      R_rcn_enddelay =R_rcn.rate_rnd[:, Time_chosen]
      R_rcn_begindelay = R_rcn.rate_rnd[:, 1]
      RCN_act_begindelay[index_simulation, :] = R_rcn_begindelay
      RCN_act_enddelay[index_simulation, :] = R_rcn_enddelay 
    ...
    # saving  
    numpy.savez_compressed(self.specF['path_sim'], Matrix_all_results=Matrix_all_results,Matrix_abs_all=Matrix_abs_all, Matrix_angle_all=Matrix_angle_all,
                           Results_ml_spikes=Results_ml_spikes, Drift_from_ml_spikes=Drift_from_ml_spikes, 
                           Matrix_initial_input=Matrix_initial_input,
                        RCN_act_begindelay=RCN_act_begindelay,
                           RCN_act_enddelay=RCN_act_enddelay,
                          Sensory_act_enddelay=Sensory_act_enddelay,
                           Sensory_act_begindelay=Sensory_act_begindelay)
